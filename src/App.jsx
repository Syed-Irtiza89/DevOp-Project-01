...
const DeploymentModal = ({ isOpen, onClose, onConfirm }) => {
  const [type, setType] = useState('Microservice (Node/React)');
  if (!isOpen) return null;

  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }} onClick={onClose}>
      <div className="glass-card" style={{ width: '100%', maxWidth: '500px', padding: '32px', position: 'relative' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <select value={type} onChange={(e) => setType(e.target.value)} style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)', borderRadius: '10px', color: 'white' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '32px' }}>
          <button onClick={onClose} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid var(--border)', borderRadius: '10px', color: 'white', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
          <button onClick={() => onConfirm(type)} style={{ flex: 1, padding: '12px', background: 'var(--primary)', border: 'none', borderRadius: '10px', color: 'white', fontWeight: 600, cursor: 'pointer' }}>Confirm Deploy</button>
        </div>
      </div>
    </div>
  );
...
const Dashboard = ({ activeView }) => {
  ...
  const [pipelines, setPipelines] = useState([
    { id: 1, name: "nexus-api-gateway", env: "PROD", status: "success", duration: "4m 12s", time: "2 mins ago" },
    { id: 2, name: "auth-service-micro", env: "STAGING", status: "running", duration: "1m 45s", time: "Just now" },
    { id: 3, name: "data-sync-worker", env: "PROD", status: "failed", duration: "12s", time: "15 mins ago" },
    { id: 4, name: "edge-cdn-handler", env: "PROD", status: "success", duration: "8m 33s", time: "1 hour ago" },
    { id: 5, name: "user-profile-v3", env: "PROD", status: "success", duration: "3m 50s", time: "3 hours ago" },
  ]);
  ...
  const handleDeploy = (type) => {
    const newPipe = {
      ...
      name: `${type.toLowerCase().replace(/ /g, '-')}-${Math.floor(Math.random() * 1000)}`,
      env: "STAGING", status: "running", duration: "0s", time: "Just now"
    };
    setPipelines([newPipe, ...pipelines]);
    setIsModalOpen(false);
    addNotification(`Initiated deployment for ${newPipe.name}`);
  };
  ...
  return (
    <div className="main-content">
      ...
      <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 2000 }}>
        {notifications.map(n => (
          <div key={n.id} className="glass-card animate-fade" style={{ background: 'var(--success)', color: 'white', padding: '12px 24px', marginBottom: '10px' }}>
            {n.msg}
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '32px 40px' }}>
        <div style={{ marginBottom: '32px' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: '8px' }}>
            {activeView === 'dashboard' ? 'Dashboard Overview' : 'Live Monitoring'}
          </h2>
          <p style={{ color: 'var(--text-muted)' }}>
            {activeView === 'dashboard' ? 'Health and performance metrics across all infrastructure clusters.' : 'Real-time telemetry and secondary performance data.'}
          </p>
        </div>
        ...
        {activeView === 'dashboard' ? (
          <>
            <div className="dashboard-grid" style={{ padding: 0 }}>
              <StatCard icon={Cpu} label="Average CPU Usage" value="42.8%" trend="+2.4%" color="59, 130, 246" />
              <StatCard icon={HardDrive} label="Active Clusters" value="12" trend="Stable" color="16, 185, 129" />
              <StatCard icon={Activity} label="Requests /sec" value="4.2k" trend="-12%" color="245, 158, 11" />
              <StatCard icon={ShieldCheck} label="Security Score" value="98/100" trend="+1" color="139, 92, 246" />
            </div>
              ...
              <div>
                <div className="glass-card animate-fade" style={{ animationDelay: '0.2s' }}>
                  <div style={{ padding: '24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 600 }}>Active Pipelines ({filteredPipelines.length})</h3>
                  ...
                  <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                    {filteredPipelines.map(p => (
                      <PipelineItem key={p.id} onClick={() => setSelectedPipeline(p.name)} {...p} />
                    ))}
                    {filteredPipelines.length === 0 && (
                      <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>No pipelines found for "{searchQuery}"</div>
                    )}
                  </div>
                </div>
          ...
          <div className="glass-card animate-fade" style={{ padding: '80px', textAlign: 'center' }}>
            <Activity size={48} color="var(--primary)" style={{ marginBottom: '20px' }} />
            <h3 style={{ fontSize: '1.5rem', marginBottom: '10px' }}>{activeView.toUpperCase()} Access Prohibited</h3>
            <p style={{ color: 'var(--text-muted)' }}>This section is currently locked in Demo Mode. Connect a live provider to unlock telemetry.</p>
          </div>
        )}
      </div>

      <DeploymentModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={handleDeploy} />
  ...
  return (
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        <nav style={{ flex: 1, marginTop: '20px' }}>
          <p style={{ padding: '0 24px 10px', fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>Quick Access</p>
          <button onClick={() => setCurrentView('dashboard')} className={`nav-link ${currentView === 'dashboard' ? 'active' : ''}`} style={{ width: '100%', background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer' }}>
            <LayoutDashboard size={20} /> Home
          </button>
          <button onClick={() => setCurrentView('monitoring')} className={`nav-link ${currentView === 'monitoring' ? 'active' : ''}`} style={{ width: '100%', background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer' }}>
            <Activity size={20} /> Monitoring
          </button>
          <button onClick={() => setCurrentView('deploy')} className={`nav-link ${currentView === 'deploy' ? 'active' : ''}`} style={{ width: '100%', background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer' }}>
            <Terminal size={20} /> Deployment Hub
          </button>

          {/* Security related stuff */}
          <p style={{ padding: '30px 24px 10px', fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>System Safety</p>
          <button className="nav-link" style={{ width: '100%', background: 'none', border: 'none', textAlign: 'left', opacity: 0.5, cursor: 'not-allowed' }}>
            <ShieldCheck size={20} /> Security Checks
          </button>
          <button className="nav-link" style={{ width: '100%', background: 'none', border: 'none', textAlign: 'left', opacity: 0.5, cursor: 'not-allowed' }}>
            <Bell size={20} /> Incident Alerts Status
          </button>
        </nav>
      </aside>

      {/* Everything else on the right */}
      <Dashboard activeView={currentView} />
    </div>
  );
}
